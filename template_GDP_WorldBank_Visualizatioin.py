...
def reconcile_countries_by_name(plot_countries,gdp_countries): #返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
	...
	Settle=set()
	Tpp=(data,Settle)
	for cao in plot_countries:
		for i in gdp_countries.values():
			if plot_countries[cao]==i["Country Name"]:
				for years in range(1960,2016):
					if i[str(years)]=="":     #如果有一年GDP不为空，则加入字典
						continue
					else:
						data[cao]=i
	for cao in plot_countries:      #所有国家代码减去字典中的为无GDP记录的国家
		if cao not in data:
			Settle.add(cao)
	return Tpp
...
def build_map_dict_by_name(gdpinfo,plot_countries, year):
	...
	with open(gdpinfo["gdpfile"],"rt") as csvfile:
		...
		for cow in reader:
			for cao in plot_countries:
				country=plot_countries[cao]
				if country==cow[gdpinfo["country_name"]] and cow[year]!="":
					Dict[cao]=math.log10(float(cow[year])) #该年有记录的就加入字典
				elif country==cow[gdpinfo["country_name"]] and cow[year]=="":
					Settle2.add(cao)       #该年无GDP记录的国家
				else:
					continue
	Settle1=Tup0[1]            #完全没有GDP记录的国家代码
	Settle3=Settle2-Settle1          #是没有某特定年记录的国家
	Tup=(Dict,Settle1,Settle3)
	return Tup
...
def test_render_world_map(year):  #测试函数
	...
	render_world_map(gdpinfo, pygal_countries, year, "qqisp_gdp_world_name_1984.svg") #调用函数
	print("文件已生成")
